...
const productOfOwner = (req, res, next) => {
  const owner_id =
    req.query.owner_id === undefined ? req.body.owner_id : req.query.owner_id;
  let limit = req.query.quantity === undefined ? undefined : req.query.quantity;
...
const transactionOfProduct = (pid, opk, bpk, tadd, status) => {
  ...
  mysqlConnection.query(query, [pid, opk, bpk, tadd, status], (err, result) => {
    if (!!err) {
      console.log(
